...
- PyInstaller 6.21.0+ (included in the venv)
...
```bash
source /home/laurent/AirRallies/fdr/bin/activate
```

Or from the fdr directory:
...
### 4. Run post-build script to separate PySide6
...
```cmd
C:\path\to\AirRallies\fdr\Scripts\activate
```
...
```bash
# Activate venv
source /path/to/AirRallies/fdr/bin/activate
...
### Alternative: Even More Compliant
...
If PyInstaller reports missing modules, add them to the spec file or use `--hidden-import`:
...
For proper Qt functionality, ensure platform plugins are included. PyInstaller's PySide6 hooks should handle this automatically, but if you have issues:

```bash
pyinstaller --add-binary "/path/to/Qt/plugins/platforms/*.so;platforms" fdr.py
```
...
Here's a basic template for GitHub Actions to build for multiple platforms:
  ...
  build-linux:
    ...
    steps:
      ...
      - name: Build with PyInstaller
        run: |
          cd fdr
          pyinstaller --onedir --name fdr --clean fdr.py
          ./post_build_linux.sh
  ...
  build-windows:
    ...
    steps:
      ...
      - name: Build with PyInstaller
        run: |
          cd fdr
          pyinstaller --onedir --name fdr --clean --windowed fdr.py
          post_build_windows.bat
...
| `post_build_linux.sh` | Post-build script for Linux (separates PySide6) |
| `post_build_windows.bat` | Post-build script for Windows (separates PySide6) |
